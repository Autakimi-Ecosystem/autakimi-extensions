...
const printHeader = () => {
  ...
  █▀▀ █▀▀ █▀▄ █▄█ █ █ ▀█  █   vRELEASE
  ${C.reset}`)
  console.log(`${C.dim}─── AutaKimi Extensions & Plugins ───────────${C.reset}\n`)
}
...
async function main() {
  try {
    ...
    // 0. Build Check
    console.log(`${C.cyan}${C.bright}Running Build Check (Extensions)...${C.reset}\n`)
    try {
      execSync('npm run build', { stdio: 'inherit', cwd: projectRoot })
      console.log(`\n${C.green}✓${C.reset} Build successful!\n`)
    } catch (error) {
      console.log(`\n${C.red}${C.bright}Build Failed!${C.reset}`)
      console.log(`${C.yellow}${C.bright}Please Fix TypeScript or build errors before releasing.${C.reset}\n`)
      return
    }
    ...
    // 3. Apply Updates to package.json
    console.log(`\n${C.cyan}${C.bright}Bumping version to v${newVersion}...${C.reset}`)
    
    rootPkg.version = newVersion
    writeFileSync(rootPkgPath, JSON.stringify(rootPkg, null, 2) + '\n')
    console.log(`${C.green}✓${C.reset} Updated package.json`)

    // 4. Re-run Build to propagate new version to templates.json and plugins.json
    console.log(`\n${C.cyan}${C.bright}Re-compiling JSON schemas with new version...${C.reset}`)
    execSync('npm run build', { stdio: 'inherit', cwd: projectRoot })
    ...
    try {
      ...
      console.log(`${C.dim}> Pushing branch ${currentBranch}...${C.reset}`)
